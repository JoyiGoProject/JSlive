...
func pwint(ws *websocket.Conn) {
	...
	for {
		...
		if err = websocket.Message.Receive(ws, &reply); err != nil {
			logger.Println("JsLiveServer:", err)
			break
		}
		for k, v := range member {
			if v.conn != ws {
...
func main() {
	//fmt.Printf("JsLiveServer is ready...\n")
	go func() {
		http.Handle("/chat", websocket.Handler(pwint))
	}()
	http.Handle("/public/", http.StripPrefix("/public/", http.FileServer(http.Dir("E://"))))
